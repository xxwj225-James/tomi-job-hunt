...
1. 下载 Releases 中的插件包 `tomihunt-extension.zip` 并解压，双击 `install-extension.bat`——按提示选择 **Chrome** 或 **Edge**（Edge 为 `edge://extensions`，开发者模式在左侧栏）
2. 在扩展管理页 → **加载已解压的扩展程序** → 选择脚本提示的固定目录（`%LOCALAPPDATA%\TomiHunt\extension`）
3. 以后更新：重新运行 `install-extension.bat`，数据自动保留
4. 点击工具栏 🤖 图标 → **⚙️ 设置** → 选择服务商（DeepSeek / Kimi / Qwen / 通用自填）→ 粘贴 API Key → **保存**（保存时会自动测试连接）
5. 打开 Boss 直聘岗位详情页 → 右下角 🤖 TomiHunt 面板自动工作；猎聘同理
6. 生成打招呼语后点「立即沟通」，在聊天页一键填入
...
**更新不会清空任何数据**——API Key、简历、JD 库、看板全部存储在代码仓库之外：

| 数据 | 位置 | 更新后 |
|---|---|---|
| 直连模式配置/简历 | 浏览器 `chrome.storage.local` | 保留 ✓ |
| 完整模式配置 | `~/.tomi-job-hunt/config.json` | 保留 ✓ |
| 简历文件 / JD 库 / 看板 / 日报 | `~/.tomi-job-hunt/` 下各文件 | 保留 ✓ |

**插件更新方式（开发模式）**：新版 zip **覆盖解压到同一个文件夹** → `chrome://extensions` 点 🔄 刷新。
⚠️ 不要解压到新的文件夹加载——Chrome 按加载目录识别插件，换目录会看不到旧数据（数据仍在，只是挂在旧目录下）。
