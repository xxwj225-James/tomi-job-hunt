...
TomiHunt 永久免费开源（无遥测、无弹窗广告）。如果你愿意支持作者持续开发，可以通过推广链接购买云服务（作者获得返佣，你的价格不变）或请作者喝咖啡——见 [docs/support.md](docs/support.md)。

## ⚠️ 使用注意

- TomiHunt 是**用户端增强工具**：只在你本人登录、本人浏览的页面上工作，不爬取、不批量请求。请遵守各招聘平台的使用条款
- 平台有账号风控机制：本插件已做请求频率控制（每次打开岗位最多 1 次接口请求），但仍无法保证平台未来调整策略后不提示验证——若出现验证提示，按平台要求完成即可
